                    ...
                    <tbody>
                        @foreach($trashItems as $item)
                            <tr class="hover:bg-graymatter-panel-light transition-colors">
                                ...
                                <td class="p-4 border-b border-divider/50 text-graymatter-lime">{{ $item->display_name }}</td>
                                ...
                                <td class="p-4 border-b border-divider/50">
                                    <div class="flex gap-2">
                                        @if(auth()->user()?->hasPermission('trash.view'))
                                            <a href="{{ route('trash.show', $item) }}" class="px-3 py-1.5 rounded-none bg-graymatter-panel-light text-graymatter-lime hover:bg-graymatter-lime hover:text-graymatter-black transition-all text-xs uppercase tracking-wider no-underline" style="font-family: var(--font-display);">
                                                View
                                            </a>
                                        @endif
                                        @if(auth()->user()?->hasPermission('trash.restore'))
                                            <form method="POST" action="{{ route('trash.restore', $item) }}">
                                                @csrf
                                                <button type="submit" class="px-3 py-1.5 rounded-none bg-graymatter-panel border border-graymatter-green/50 text-graymatter-green hover:bg-graymatter-green hover:text-graymatter-black hover:border-graymatter-green transition-all text-xs uppercase tracking-wider" style="font-family: var(--font-display);">
                                                    Restore
                                                </button>
                                            </form>
                                        @endif
