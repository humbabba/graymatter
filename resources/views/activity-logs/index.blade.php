    ...
    <div class="p-4 md:p-8">
        ...
        <div class="mb-6 p-4 bg-graymatter-panel-light rounded-sm">
            <form method="GET" action="{{ route('activity-logs.index') }}" class="flex flex-wrap gap-4 items-end">
                ...
                <div>
                    <label for="type" class="block font-semibold text-graymatter-teal uppercase text-sm tracking-wider mb-2" style="font-family: var(--font-display);">Type</label>
                    <select name="type" id="type" class="bg-graymatter-panel border border-divider rounded-sm px-4 py-2 text-text min-w-[150px]">
                ...
                <div>
                    <label for="action" class="block font-semibold text-graymatter-teal uppercase text-sm tracking-wider mb-2" style="font-family: var(--font-display);">Action</label>
                    <select name="action" id="action" class="bg-graymatter-panel border border-divider rounded-sm px-4 py-2 text-text min-w-[150px]">
                ...
                <div>
                    <label for="user" class="block font-semibold text-graymatter-teal uppercase text-sm tracking-wider mb-2" style="font-family: var(--font-display);">User</label>
                    <select name="user" id="user" class="bg-graymatter-panel border border-divider rounded-sm px-4 py-2 text-text min-w-[150px]">
                            ...
                            <tr class="hover:bg-graymatter-panel-light transition-colors">
                                ...
                                <td class="p-4 border-b border-divider/50">
                                    @if($log->loggableExists())
                                        ...
                                        @endphp
                                        @if($routeName)
                                            <a href="{{ route($routeName, $log->loggable_id) }}" class="text-graymatter-lime hover:text-graymatter-green">
                                                {{ $log->getLoggableDisplayName() }}
                                            </a>
                                        @else
                                            <span class="text-graymatter-lime">{{ $log->getLoggableDisplayName() }}</span>
                                        @endif
                                ...
                                <td class="p-4 border-b border-divider/50">
                                    @if($log->user_id)
                                        @if($log->userExists())
                                            <a href="{{ route('users.edit', $log->user_id) }}" class="text-graymatter-teal hover:text-graymatter-lime">
                                                {{ $log->user_name }}
                                            </a>
                                        @else
                                ...
                                <td class="p-4 border-b border-divider/50">
                                    ...
                                    @if(auth()->user()?->hasPermission('activity-logs.view'))
                                        <a href="{{ route('activity-logs.show', $log) }}" class="px-3 py-1.5 rounded-none bg-graymatter-panel-light text-graymatter-lime hover:bg-graymatter-lime hover:text-graymatter-black transition-all text-xs uppercase tracking-wider no-underline ml-2" style="font-family: var(--font-display);">
                                            View
                                        </a>
                                    @endif
                                </td>
        ...
        @else
            <div class="text-center py-16 text-text-muted">
                <div class="w-16 h-16 mx-auto mb-4 rounded-none bg-graymatter-panel-light flex items-center justify-center">
                    <svg class="w-8 h-8 text-graymatter-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24">
