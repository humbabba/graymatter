                    ...
                    @foreach($roles as $role)
                        <tr class="hover:bg-graymatter-panel-light transition-colors">
                            <td class="p-4 border-b border-divider/50">
                                <span class="text-graymatter-green font-semibold">{{ $role->name }}</span>
                            </td>
                            <td class="p-4 border-b border-divider/50 text-text-muted">{{ $role->description }}</td>
                            <td class="p-4 border-b border-divider/50">
                                <div class="flex flex-wrap gap-1">
                                    @foreach($role->permissions->sortBy('name')->groupBy(fn($p) => explode('.', $p->name)[0]) as $group => $perms)
                                        <span class="badge badge-lime" title="{{ $perms->map(fn($p) => explode('.', $p->name)[1] ?? $p->name)->implode(', ') }}">
                                            {{ $group }} ({{ $perms->count() }})
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <td class="p-4 border-b border-divider/50">
                                <div class="flex gap-2">
                                    @if(auth()->user()?->hasPermission('roles.edit'))
                                        <a href="{{ route('roles.edit', $role) }}" class="px-3 py-1.5 rounded-none bg-graymatter-panel-light text-graymatter-teal hover:bg-graymatter-teal hover:text-graymatter-black transition-all text-xs uppercase tracking-wider no-underline" style="font-family: var(--font-display);">
                                            Edit
                                        </a>
                                    @endif
                                    @if(auth()->user()?->hasPermission('roles.create'))
                                        <form method="POST" action="{{ route('roles.copy', $role) }}">
                                            @csrf
                                            <button type="submit" class="px-3 py-1.5 rounded-none bg-graymatter-panel-light text-graymatter-green hover:bg-graymatter-green hover:text-graymatter-black transition-all text-xs uppercase tracking-wider" style="font-family: var(--font-display);">
                                                Copy
                                            </button>
                                        </form>
                                    @endif
